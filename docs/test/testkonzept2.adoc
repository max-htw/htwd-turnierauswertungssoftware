...
| T13 | Speichern und Laden eines Turniers (SQLite)              | DB-SQLite         | Nein
| T14 | Laden gespeicherter Turnierdaten                         | DB-SQLite          | Nein
| T15 | Generierung der Gesamttabelle (Service + UI)             | SpielplanService   | Nein
| T16 | PDF-Export des Spielplans                                | PDF-Export        | Nein
| T17 | CSV-Export der Turnierdaten                              | CSV-Export       | Nein
|===
...
Unit-Integration: H2-InMemory-DB, JUnit 5. Frameworks wie Mockito für Mocking zur Unit-Test-Isolierung werden in Überlegung gezogen, sind jedoch bisher noch nicht implementiert.

Datenbank: H2-InMemory-DB & SQLite.
...
Dokumentation: Ergebnisse in Testprotokollen (Asciidoc/Excel) und im Git-Repository ablegen.
...
CI-Pipeline liefert automatisierte Reports (JUnit-XML).

Fehler werden im Issue-Tracker (z. B. Jira) mit Verweis auf Testfall-IDs dokumentiert.
