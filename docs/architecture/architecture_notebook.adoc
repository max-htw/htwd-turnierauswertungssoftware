...
Wegen widersprüchlichen Kundenanforderungen an die Clientanzahl, wählen wir zwei Sätze von Architekturkomponenten aus: einmal für 20 Clients und einmal für 200 Clients. Beim Entwurf der Komponentenschnittstellen berücksichtigen wir eine spätere Möglichkeit der Hochskallierung des Systems durch Austausch von Komponenten.

=== Auswahl der zuverwendenden Komponenten für 20 Clients
. WLAN-Netzwerk wird mithilfe der in dem Notebook von Organisatoren eingebauten WLAN-Karte gewährleistet und durch das Linux-Betriebssystem verwaltet.

. Das Software-Produkt wird als eine Java-Anwendung in form von JAR-Datei ausgeliefert.
. Die Turnierdaten werden zum Archivieren in einfache Dateien geschrieben, beim Programmstart aus den Dateien geladen und intern in den Datenstrukturen von Java (ArrayList, HashMap) verarbeitet.
. Das Front-End, das die HTML-Code generiert und die Nutzereingaben aus dem Http-Request auswertet programmieren wir selbst.

=== Auswahl der zuverwendenden Komponenten für 200 Clients
. Das WLAN-Netzwerk muss aus einem oder mehreren externen WiFi-Access-Points bestehen. Die Einrichtung des Netzwerks gehört nicht zu den Funktionen unseres Softwaresystems.
