...
. Das Software-Produkt wird später mithilfe von https://www.docker.com[Docker] ausgeliefert, damit sichergestellt ist, dass jedes System die gleichen Voraussetzungen erfüllt.
. Zur Speicherung der Daten wird https://www.postgresql.org[PostgreSQL] benutzt, da die Speicherung in Datenbanken übersichtlicher und robuster ist, als die Speicherung der Daten in Dateien.
// . Für die Entwicklung im Front-End wird https://svelte.dev/docs/kit/introduction[SvelteKit] benutzt. Dies vereinfacht die Nutzung von API's und bietet ein Übersichtliches Routing-System an.
...
==== C4 Context
plantuml::{plantumlsdir}/an_c4_context.puml[svg]

==== C4 Container
plantuml::{plantumlsdir}/an_c4_container.puml[svg]
