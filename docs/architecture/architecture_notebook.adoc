...
Ein weiteres Ziel ist, dass die Software später auch von anderen weiterentwickelt werden kann. Dafür benutzen wir bekannte Tools und Technologien wie Docker, PostgreSQL und Java, die im Informatik-Studium schon verwendet werden. So soll sichergestellt werden, dass die Anwendung leicht wartbar ist und auch bei zukünftigen Turnieren genutzt oder angepasst werden kann.
...
// Hinweis: Wir verwenden keine Use Cases, sondern arbeiten mit User Stories, die im Product Backlog gepflegt werden.
