...
Wegen widersprüchlichen Kundenanforderungen an die Clientanzahl, wählen wir zwei Sätze von Architekturkomponenten aus: einmal für 20 Clients und einmal für gleichzeitig aktive 200 Clients. Beim Entwurf der Komponentenschnittstellen berücksichtigen wir eine spätere Möglichkeit der Hochskalierung des Systems durch Austausch von Komponenten.
...
. Das Front-End, das den HTML-Code generiert und die Nutzereingaben aus dem HTTP-Request auswertet, programmieren wir selbst.
